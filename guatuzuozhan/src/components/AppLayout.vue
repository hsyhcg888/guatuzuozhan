<script setup>
import { ref } from 'vue'; import { useRouter } from 'vue-router'; import { useAuthStore } from '@/stores/auth'; import Sidebar from './Sidebar.vue'; import TopBar from './TopBar.vue'; import ConfirmModal from './ConfirmModal.vue'
const auth=useAuthStore();const router=useRouter();const collapsed=ref(false);const logoutOpen=ref(false);async function logout(){auth.logout();logoutOpen.value=false;await router.replace('/login')}
</script>
